<template>
  <view class="profile-settings-page">
    <view class="hero-card">
      <text class="eyebrow">PLAYER PROFILE</text>
      <text class="hero-title">陪玩资料设置</text>
      <text class="hero-sub">简介和语音修改提交审核，审核通过后才替换公开资料</text>
    </view>

    <view v-if="settings?.pending_update" class="review-card review-pending">
      <text>资料审核中</text>
      <text>提交时间：{{ dateTime(settings.pending_update.submitted_at) }}</text>
      <text>审核期间，老板端仍然展示你当前已通过的资料。</text>
    </view>
    <view v-else-if="settings?.latest_update?.status === 'rejected'" class="review-card review-rejected">
      <text>上次修改未通过</text>
      <text>{{ settings.latest_update.reject_reason || '请修改后重新提交' }}</text>
    </view>

    <view class="card current-card">
      <view class="card-head">
        <view><text class="card-title">当前公开资料</text><text class="card-sub">陪玩类型只能由管理员调整</text></view>
        <text class="type-chip">{{ settings?.player.type_name || '-' }}</text>
      </view>
    ...
    <view class="card form-card">
      <view class="card-head">
        <view><text class="card-title">提交新资料</text><text class="card-sub">新的申请会覆盖尚未审核的修改稿</text></view>
        <text class="count-chip">{{ form.bio.length }}/500</text>
      </view>
      ...
      <textarea
        ...
        class="bio-input"
        maxlength="500"
        placeholder="填写在线时间、擅长玩法、沟通风格等公开介绍"
      />

      <view class="audio-section">
        <view class="audio-head"><text>语音自我介绍</text><text>支持 MP3 / M4A / AAC / WAV，20MB以内</text></view>
        <view v-if="form.audio_intro_url" class="audio-card">
          <view class="audio-icon">声</view>
          <view class="audio-main"><text>{{ form.audio_intro_title || '音频自我介绍' }}</text><text>{{ isPlaying ? '正在播放' : '已上传，等待随资料提交审核' }}</text></view>
          <button class="audio-action" @tap="toggleAudio">{{ isPlaying ? '暂停' : '试听' }}</button>
          <button class="audio-remove" @tap="removeAudio">移除</button>
        </view>
        <button v-else class="upload-btn" :disabled="audioUploading" @tap="chooseAudio">
          {{ audioUploading ? '上传中...' : '选择并上传语音介绍' }}
        </button>
      </view>

      <view class="review-notice">
        <text>审核说明</text>
        <text>{{ settings?.review_notice || '修改提交后进入审核，审核期间继续展示旧资料。' }}</text>
      </view>
      <button class="submit-btn" :disabled="submitting || loading || audioUploading" @tap="submitProfile">
        {{ submitting ? '提交中...' : (settings?.pending_update ? '更新待审核资料' : '提交审核') }}
      </button>
    </view>
...
import {
  ...
  submitPlayerProfileUpdate,
  type PlayerProfileSettingsResult
} from '@/api/player'
...
async function loadSettings() {
  loading.value = true
  try {
    applyForm(await getPlayerProfileSettings())
  } catch (error) {
    toast(getErrorMessage(error, '陪玩资料加载失败'))
  } finally {
    loading.value = false
  }
}
  ...
  chooseFile({
    ...
    success: async (res: any) => {
      ...
      try {
        ...
        toast(getErrorMessage(error, '语音上传失败'))
      } finally {
...
function getAudioContext() {
  if (!form.audio_intro_url) return null
  if (!audioContext) {
    ...
    audioContext.onError(() => { isPlaying.value = false; toast('语音播放失败') })
  }
  audioContext.src = form.audio_intro_url
  return audioContext
}
...
async function removeAudio() {
  if (!(await confirm('移除后，审核通过时公开资料中的语音也会被清除。确定移除吗？', '移除语音'))) return
...
async function submitProfile() {
  ...
  if (!form.bio.trim() && !form.audio_intro_url) {
    if (!(await confirm('当前简介和语音均为空，审核通过后公开资料也会清空。仍要提交吗？', '确认提交'))) return
  }
  submitting.value = true
  try {
    const result = await submitPlayerProfileUpdate({
      bio: form.bio.trim(),
      audio_intro_url: form.audio_intro_url.trim(),
      audio_intro_title: form.audio_intro_title.trim()
    })
    applyForm(result)
    success('资料修改已提交审核')
  } catch (error) {
    toast(getErrorMessage(error, '资料提交失败'))
  } finally {
