  ...
  <view class="club-page apply-page">
    ...
    <view class="form-card audio-card">
      <view class="form-head">
        ...
        <view>
          <text class="form-eyebrow">音频自我介绍</text>
          <text class="form-hint">可选，上传后审核通过会展示在陪玩详情页</text>
        </view>
      </view>
      ...
      <view v-if="form.audio_intro_url" class="audio-info">
        ...
        <view class="audio-main">
          <text class="audio-title">{{ form.audio_intro_title || '音频自我介绍' }}</text>
          <text class="audio-url">已上传，提交申请后同步给后台审核</text>
        </view>
        <text class="audio-remove" @tap="removeAudio">移除</text>
      </view>

      <button v-else class="audio-upload" :disabled="audioUploading" @tap="chooseAudio">
        {{ audioUploading ? '上传中...' : '上传音频介绍' }}
      </button>
    ...
    <view class="footer-actions">
      <button class="club-btn club-btn--primary" :disabled="submitting || audioUploading" @tap="submitApply">
        {{ submitting ? '提交中...' : '提交申请' }}
      </button>
    </view>
...
function chooseAudio() {
  ...
  if (!chooseFile) {
    toast('当前环境不支持选择音频文件，请在微信开发者工具或真机中使用')
    return
  }
  chooseFile({
    count: 1,
    ...
    success: async (res: any) => {
      ...
      if (!/\.(mp3|m4a|aac|wav)$/.test(lower)) {
        toast('只支持 MP3/M4A/AAC/WAV 音频')
        return
      }
...
.apply-hero {
  ...
  background: linear-gradient(135deg, #fff8ed 0%, #fef5dc 56%, #edf6f0 100%);
}
