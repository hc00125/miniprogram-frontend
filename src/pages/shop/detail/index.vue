    ...
    <scroll-view v-if="product" scroll-y class="detail-scroll">
      <view class="hero-section">
        ...
        <view v-else class="hero-placeholder">
          ...
          <view class="hero-note">按规格确认后下单，客服接单后开局</view>
        </view>
      ...
      <view class="detail-card option-card">
        ...
        <view v-if="specs.length" class="spec-section">
          <view class="spec-title">规格明细</view>
...
const detailText = computed(() => product.value?.description || selectedSpec.value?.description || '选择规格后确认下单')
const memberStripText = computed(() => product.value?.product_type === 'guarantee' ? '保底规格可选，价格按单计算' : '加入会员，享会员价')
...
.product-name { display: block; margin-top: 18rpx; color: #2b2b2b; font-size: 34rpx; font-weight: 900; line-height: 1.35; }
...
.spec-title { margin-bottom: 14rpx; color: #333; font-size: 26rpx; font-weight: 900; }
