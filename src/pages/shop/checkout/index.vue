<template>
  <view class="checkout-page">
    <scroll-view scroll-y class="checkout-scroll">
      <view v-if="isCartCheckout" class="card merge-card">
        <view class="card-head">
          <view>
            <text class="card-title">购物车结算</text>
            <text class="card-subtitle">本次选择 {{ cartItems.length }} 个商品</text>
          </view>
      ...
      <view v-else-if="product" class="card product-card">
        ...
        <view class="product-main">
          ...
          <view class="price-row">
            <text>¥</text><text>{{ formatMoney(basePrice) }}</text><text>{{ isSpecProduct ? '/单' : '/时/人' }}</text>
          </view>
        </view>
      </view>
      ...
      <view v-if="!isCartCheckout && product && specs.length" class="card">
        <view class="card-head">
          <view>
            ...
            <text class="card-subtitle">固定价格规格可直接使用微信虚拟支付</text>
          </view>
      ...
      <view v-if="hasCheckoutData" class="pay-rule-card" :class="{ blocked: Boolean(virtualPayBlockReason) }">
        ...
        <view class="rule-main">
          <text class="rule-title">{{ virtualPayBlockReason ? '当前组合暂不能使用虚拟支付' : '可使用微信官方虚拟支付' }}</text>
          <text class="rule-desc">{{ virtualPayBlockReason || virtualPayReadyText }}</text>
        </view>
      </view>
      ...
      <view v-if="hasCheckoutData" class="card">
        ...
        <view class="field">
          ...
          <input v-model="form.contact" class="field-input" placeholder="请输入您的联系昵称" />
        </view>
        <view class="field">
          <text class="field-label">游戏ID / 队伍码</text>
          <input v-model="form.gameId" class="field-input" placeholder="请输入游戏ID或队伍码" />
        </view>
        <view v-if="!isCartCheckout" class="field">
          <text class="field-label">购买数量</text>
          <view class="stepper">
            <button :disabled="form.quantity <= 1" @tap="adjustQuantity(-1)">−</button>
            <text>{{ form.quantity }}</text>
            <button class="plus" @tap="adjustQuantity(1)">＋</button>
          </view>
        ...
        <view v-if="!isCartCheckout && !isSpecProduct" class="dynamic-grid">
          <view class="field">
            ...
            <view class="stepper">
              ...
              <text>{{ form.playerCount }}</text>
              <button class="plus" @tap="adjustPlayerCount(1)">＋</button>
            </view>
          </view>
          <view class="field">
            <text class="field-label">预订时长</text>
            <view class="stepper">
        ...
        <view class="field textarea-field">
          ...
          <textarea v-model="form.note" class="field-textarea" maxlength="80" placeholder="如有其他需求请备注（选填）" />
      ...
      <view v-if="hasCheckoutData" class="card amount-card">
        ...
        <template v-if="isCartCheckout">
          <view class="amount-line"><text>商品项目</text><text>{{ cartItems.length }}项</text></view>
    ...
    <view v-if="hasCheckoutData" class="bottom-bar">
      ...
      <button
        class="submit-btn"
        :disabled="submitting || Boolean(virtualPayBlockReason)"
        @tap="submitOrder"
      >
        {{ submitting ? '提交中...' : (virtualPayBlockReason ? '当前组合不可下单' : '立即下单') }}
      </button>
    </view>
  </view>
</template>
...
const productDesc = computed(() => {
  if (isGuaranteeProduct.value) return selectedSpec.value ? `当前规格：${selectedSpec.value.guarantee_amount || selectedSpec.value.name}` : '请选择固定价格保底规格。'
...
const virtualPayBlockReason = computed(() => {
  ...
  if (isCartCheckout.value && cartItems.value.length !== 1) {
    return '当前阶段暂不支持多个不同商品合并虚拟支付，请返回购物车仅选择一个商品规格。'
  }
  if (!isCartCheckout.value && !isSpecProduct.value) {
    return '当前阶段暂不支持按人数和服务时长动态计价，请选择带固定价格规格的商品。'
  }
  return ''
})
...
const virtualPayReadyText = computed(() => {
  ...
  return selectedSpec.value
    ? `已选择“${selectedSpec.value.name}”，支付时将按该规格与购买数量核对金额。`
    : '请选择一个固定价格规格，支付时会自动核对微信道具与订单金额。'
})
...
function itemSpecId(item: ShopCartItem) {
  const specId = Number(item.spec_id || item.spec_id_snapshot || 0)
  return Number.isFinite(specId) && specId > 0 ? specId : null
}
function getProductImage(item: BossPackage) {
  const current = item as BossPackage & Record<string, any>
  return current.cover_url || current.image_url || current.thumb_url || current.picture_url || fallbackImage
}
function getDisplayPrice(item: BossPackage) {
  const prices = (item.specs || []).map(spec => Number(spec.price || 0)).filter(price => price >= 0)
  return prices.length ? Math.min(...prices) : getProductPrice(item)
}
function getProductPrice(item: BossPackage) {
  const current = item as BossPackage & Record<string, any>
  return Math.max(0, Number(current.price ?? current.base_price ?? 0))
}
...
async function ensureBossOpenid() {
  const local = getBossOpenid()
  if (local) return local
  try { return getBossOpenid(await syncClientProfile()) } catch { return '' }
}
...
function showUnfinishedOrders(orders: BossOrderListItem[]) {
  const lines = orders.slice(0, 5).map(order => `${order.order_no}（${order.status}）`).join('\n')
...
function syncSelectedSpec(nextSpecs = specs.value) {
  ...
  selectedSpec.value = nextSpecs.find(item => String(item.id) === initialSpecId.value) || nextSpecs[0]
}
async function fetchCartCheckout() {
  loading.value = true
  try {
    const selectedIds = new Set(cartItemIds.value)
    cartItems.value = (await getShopCart()).filter(item => selectedIds.has(String(item.id)))
...
async function fetchProduct() {
  ...
  loading.value = true
  try {
    const matched = (await getPackages()).find(item => item.id === packageId.value) || null
...
function adjustQuantity(delta: number) {
  ...
  if (next < 1) return
  if (next > 99) return toast('单次最多选择99件')
  form.quantity = next
}
...
async function showVirtualPayBlock() {
  if (!virtualPayBlockReason.value) return
  await new Promise(resolve => uni.showModal({
    title: '当前组合暂不可下单',
    content: `${virtualPayBlockReason.value}\n\n同一固定价格规格可通过修改数量购买多份。`,
    showCancel: false,
    confirmText: '我知道了',
    confirmColor: '#2f9b63',
    complete: resolve
...
.card-title { display: block; color: #172116; font-size: 30rpx; font-weight: 900; }
...
.product-name { flex: 1; font-size: 31rpx; font-weight: 900; line-height: 1.35; }
.product-tag { flex-shrink: 0; padding: 6rpx 12rpx; border-radius: 999rpx; color: #a87520; font-size: 20rpx; font-weight: 900; background: #fff6df; }
.product-desc { margin-top: 10rpx; color: #7d877a; font-size: 23rpx; line-height: 1.45; }
.price-row { margin-top: auto; display: flex; align-items: baseline; color: #a87520; }
.price-row text:nth-child(1) { font-size: 25rpx; font-weight: 900; }
.price-row text:nth-child(2) { font-size: 42rpx; font-weight: 900; }
.price-row text:nth-child(3) { margin-left: 6rpx; font-size: 21rpx; }
.merge-card { background: linear-gradient(135deg, #fff, #fffaf0); }
.merge-list { display: flex; flex-direction: column; gap: 14rpx; }
.merge-item { display: flex; align-items: center; gap: 14rpx; padding: 16rpx; border-radius: 20rpx; background: rgba(247,250,244,.86); }
.item-image { width: 104rpx; height: 104rpx; flex-shrink: 0; border-radius: 18rpx; }
.item-image--placeholder { display: flex; align-items: center; justify-content: center; color: #fff; font-size: 36rpx; font-weight: 900; background: #2f9b63; }
.item-main { flex: 1; min-width: 0; }
.item-name, .item-spec, .item-price { display: block; }
.item-name { font-size: 26rpx; font-weight: 900; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; }
.item-spec { margin-top: 5rpx; color: #a87520; font-size: 21rpx; }
.item-price { margin-top: 5rpx; color: #7d877a; font-size: 21rpx; }
.item-total { color: #a87520; font-size: 25rpx; font-weight: 900; }
...
.spec-chip text:nth-child(2) { margin-top: 7rpx; color: #7d877a; font-size: 21rpx; }
.spec-chip text:last-child { margin-top: 9rpx; color: #a87520; font-size: 29rpx; font-weight: 900; }
.spec-chip.active { border-color: #2f9b63; background: #eef8f1; box-shadow: 0 8rpx 20rpx rgba(47,155,99,.10); }
...
.stepper button { width: 52rpx; height: 52rpx; display: flex; align-items: center; justify-content: center; padding: 0; margin: 0; border-radius: 50%; color: #536151; background: #fff; }
.stepper button::after { border: none; }
.stepper button.plus { color: #fff; background: #2f9b63; }
.stepper button[disabled] { opacity: .4; }
.stepper text { font-size: 26rpx; font-weight: 900; }
...
.empty-state { min-height: 70vh; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 20rpx; color: #879083; font-size: 26rpx; }
.empty-state button { min-width: 180rpx; border-radius: 999rpx; color: #fff; background: #2f9b63; }
...
.bottom-price text:last-child { margin-top: 3rpx; color: #a87520; font-size: 34rpx; font-weight: 900; }
.submit-btn { min-width: 236rpx; height: 82rpx; display: flex; align-items: center; justify-content: center; padding: 0 28rpx; margin: 0; border-radius: 999rpx; color: #fff; font-size: 27rpx; font-weight: 900; background: linear-gradient(135deg, #5fc68a, #1f7c4b); }
