  ...
  <view class="page">
    ...
    <view class="body">
      ...
      <scroll-view scroll-y class="content" refresher-enabled :refresher-triggered="refreshing" @refresherrefresh="refreshData">
        <view class="inner">
          <view class="banner"><text>TOUCHI CLUB</text><strong>{{ activeCategoryName }}</strong><small>统一服务标准 · 官方虚拟支付 · 订单全程留痕</small></view>
...
async function load(silent = false) { if (!silent) loading.value = true; try { const [groups, list] = await Promise.all([getPackageGroups(), getPackages()]); categories.value = normalizeGroups(groups, list); products.value = list; if (!categories.value.some(item => item.id === activeCategoryId.value)) activeCategoryId.value = categories.value[0].id; loaded.value = true } catch (error) { toast(getErrorMessage(error, '商品加载失败')) } finally { loading.value = false; refreshing.value = false } }
