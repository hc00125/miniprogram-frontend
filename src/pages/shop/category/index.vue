            ...
            >
              ...
              <view class="product-info">
                <view class="product-name-row">
                  ...
                  <text v-if="product.product_type === 'guarantee'" class="product-tag">规格</text>
                </view>
                ...
                <view class="product-bottom">
                  <view class="price-wrap">
                    <text class="price-prefix">{{ product.specs?.length ? '起' : '' }}</text>
                    <text class="price-symbol">¥</text>
                    <text class="price-value">{{ formatMoney(getProductPrice(product)) }}</text>
                    <text class="price-unit">{{ product.product_type === 'guarantee' ? '/单' : '/时' }}</text>
                  </view>
...
const activeCategory = computed(() => categories.value.find(item => item.id === activeCategoryId.value) || categories.value[0] || fallbackCategory)
const activeCategoryName = computed(() => activeCategory.value?.name || '商品分类')
...
function getProductPrice(product: BossPackage) {
  ...
  if (specs.length) return Math.min(...specs.map(item => Number(item.price || 0)).filter(price => price >= 0))
  const item = product as BossPackage & Record<string, any>
...
.search-icon { color: #9d9d9d; font-size: 34rpx; line-height: 1; }
.search-input { flex: 1; height: 64rpx; color: #222; font-size: 28rpx; }
.top-category-scroll { width: 100%; margin-top: 22rpx; white-space: nowrap; }
.top-category-list { display: inline-flex; gap: 34rpx; padding: 2rpx 2rpx 8rpx; }
.top-category-item { display: inline-flex; flex-direction: column; align-items: center; gap: 10rpx; min-width: 112rpx; }
.top-category-icon { width: 76rpx; height: 76rpx; display: flex; align-items: center; justify-content: center; border-radius: 50%; color: #1f7c4b; font-size: 34rpx; font-weight: 900; background: linear-gradient(135deg, #f2fff5, #dff5e5); border: 4rpx solid transparent; box-sizing: border-box; }
.top-category-item.active .top-category-icon { color: #fff; background: linear-gradient(135deg, #f06575, #ec3f51); border-color: rgba(236, 63, 81, 0.18); }
.top-category-name { max-width: 150rpx; color: #333; font-size: 26rpx; line-height: 1.2; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.top-category-item.active .top-category-name { color: #ec3f51; font-weight: 800; }
.shop-body { display: flex; height: calc(100vh - 212rpx - env(safe-area-inset-bottom)); min-height: 760rpx; border-top: 1rpx solid #f3f3f3; background: #fff; }
.side-category-scroll { width: 190rpx; height: 100%; flex-shrink: 0; background: #f7f7f7; }
.side-category-item { position: relative; min-height: 112rpx; display: flex; align-items: center; justify-content: center; padding: 18rpx; box-sizing: border-box; color: #333; font-size: 28rpx; line-height: 1.32; text-align: center; }
.side-category-item.active { color: #e94b5a; font-weight: 900; background: #fff; }
.side-category-item.active::before { content: ''; position: absolute; left: 0; top: 24rpx; bottom: 24rpx; width: 6rpx; border-radius: 999rpx; background: #e94b5a; }
.product-scroll { flex: 1; height: 100%; background: #fff; }
.product-content { padding: 18rpx 22rpx 32rpx; box-sizing: border-box; }
.promo-banner { min-height: 132rpx; display: flex; align-items: center; padding: 24rpx 26rpx; margin-bottom: 20rpx; border-radius: 12rpx; overflow: hidden; box-sizing: border-box; color: #fff; background: radial-gradient(circle at 80% 20%, rgba(255, 255, 255, 0.22), transparent 30%), linear-gradient(135deg, #2e313d 0%, #151722 48%, #cf3147 100%); }
.promo-eyebrow, .promo-title, .promo-sub { display: block; }
.promo-eyebrow { font-size: 20rpx; font-weight: 900; opacity: 0.72; }
.promo-title { margin-top: 8rpx; font-size: 32rpx; font-weight: 900; }
.promo-sub { margin-top: 8rpx; font-size: 22rpx; opacity: 0.82; }
.state-card { padding: 120rpx 20rpx; color: #888; font-size: 26rpx; text-align: center; }
.product-list { display: flex; flex-direction: column; }
.product-card { display: flex; gap: 20rpx; padding: 20rpx 0; border-bottom: 1rpx solid #ededed; }
.product-image { width: 150rpx; height: 150rpx; flex-shrink: 0; border-radius: 8rpx; background: #f3f3f3; }
.product-info { min-width: 0; flex: 1; display: flex; flex-direction: column; }
.product-name-row { display: flex; align-items: center; gap: 10rpx; }
.product-name { color: #2f2f2f; font-size: 31rpx; font-weight: 500; line-height: 1.32; }
.product-tag { padding: 4rpx 10rpx; border-radius: 999rpx; color: #ef4f5f; font-size: 20rpx; background: #fff1f3; }
.product-desc { margin-top: 8rpx; color: #8a8a8a; font-size: 23rpx; line-height: 1.3; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; }
.product-sold { margin-top: 10rpx; color: #999; font-size: 23rpx; }
.product-bottom { margin-top: auto; display: flex; align-items: flex-end; justify-content: space-between; gap: 20rpx; }
.price-wrap { display: flex; align-items: baseline; color: #ef4f5f; }
.price-prefix { margin-right: 4rpx; color: #ef4f5f; font-size: 20rpx; }
.price-symbol { font-size: 26rpx; font-weight: 800; }
.price-value { margin-left: 4rpx; font-size: 44rpx; font-weight: 900; line-height: 1; }
.price-unit { margin-left: 4rpx; color: #ef4f5f; font-size: 22rpx; }
.cart-btn { width: 58rpx; height: 58rpx; display: flex; align-items: center; justify-content: center; padding: 0; margin: 0; border-radius: 50%; color: #e94b5a; font-size: 28rpx; line-height: 1; background: #fff; border: 2rpx solid #ef6b77; }
