  ...
  <view class="cart-page">
    <scroll-view scroll-y class="cart-scroll">
      <view class="cart-header">
        <view>
          ...
          <text class="cart-subtitle">{{ loading ? '正在同步购物车...' : `共 ${cartCount} 件商品，可同时发布多个独立订单` }}</text>
        </view>
      ...
      <view v-if="items.length" class="select-row" @tap="toggleSelectAll">
        <view class="select-all-main">
          <view class="check-circle" :class="{ checked: allSelected }">✓</view>
          <text>{{ allSelected ? '取消全选' : '全选' }}</text>
        </view>
        <text>每件商品、每个数量都会发布为独立订单</text>
      </view>
      ...
      <view v-else class="empty-state">
        ...
        <text class="empty-desc">{{ loading ? '请稍等' : '先去挑选套餐和所需陪玩类型吧' }}</text>
    ...
    <view v-if="items.length" class="bottom-bar">
      <view class="total-box">
        <text>{{ selectedItems.length ? `已选 ${selectedItems.length} 项，将发布 ${selectedOrderCount} 个订单` : '请选择需要结算的商品' }}</text>
...
const selectedOrderCount = computed(() => selectedItems.value.reduce((sum, item) => sum + Number(item.quantity || 1), 0))
...
function syncSelectedIds(nextItems: ShopCartItem[]) {
  const available = new Set(nextItems.map(itemKey))
...
function checkoutItem(item: ShopCartItem) {
  go('/pages/shop/checkout/index', { cartItemIds: itemKey(item) })
}
function checkoutSelected() {
  if (!selectedItems.value.length) return toast('请选择需要结算的商品')
  if (selectedOrderCount.value > 20) return toast('单次最多发布20个订单，请减少商品数量')
...
.select-row { display:flex;align-items:center;justify-content:space-between;gap:18rpx;margin:8rpx 22rpx 0;padding:18rpx 20rpx;border-radius:18rpx;background:#fff;box-sizing:border-box; }.select-all-main { display:flex;align-items:center;gap:12rpx;flex-shrink:0; }.select-all-main text { color:#333;font-size:25rpx;font-weight:900; }.select-row>text { color:#999;font-size:21rpx;text-align:right; }
...
.card-actions { position:absolute;right:18rpx;top:22rpx;display:flex;flex-direction:column;gap:10rpx; }.card-actions button { width:106rpx;height:52rpx;margin:0;padding:0;border-radius:999rpx;font-size:20rpx; }.remove-btn { color:#888;background:#f3f3f3; }.buy-btn { color:#fff;background:#1f7c4b; }
