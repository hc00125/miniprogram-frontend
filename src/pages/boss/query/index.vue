<template>
  <view class="club-page query-page">
    ...
    <view class="query-hero">
      ...
      <button class="refresh-btn" @tap="fetchOrders">
...
function coverStatusClass(status: string) {
  return `cover-status--${coverStatusKey(status)}`
}
...
.tab.active {
  ...
  box-shadow: 0 8rpx 18rpx rgba(31, 124, 75, 0.22);
}
...
.order-card {
  ...
  background: rgba(255, 255, 255, 0.96);
  overflow: hidden;
  box-shadow: 0 14rpx 32rpx rgba(38, 69, 54, 0.06);
}
...
.cover-shade {
  ...
  inset: 0;
  background:
    linear-gradient(180deg, rgba(0, 0, 0, 0) 30%, rgba(0, 0, 0, 0.30) 100%),
    linear-gradient(90deg, rgba(255, 255, 255, 0.10), transparent 30%);
}
...
.player-avatar {
  ...
  box-shadow: 0 4rpx 10rpx rgba(31, 124, 75, 0.18);
  margin-left: -8rpx;
}
...
.empty-ring {
  ...
  inset: 0;
  border-radius: 50%;
  border: 2rpx solid rgba(47, 155, 99, 0.18);
}
...
.empty-ring--inner {
  inset: 22rpx;
  border-color: rgba(47, 155, 99, 0.28);
...
.empty-icon {
  ...
  height: 96rpx;
  border-radius: 28rpx;
  background: linear-gradient(135deg, #fef5dc, #fff7e6);
  color: #a87520;
  font-size: 38rpx;
  font-weight: 900;
  display: flex;
...
.loading-spinner {
  width: 56rpx;
  height: 56rpx;
  border: 4rpx solid rgba(47, 155, 99, 0.18);
  border-top-color: #1f7c4b;
  border-radius: 50%;
