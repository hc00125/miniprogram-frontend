<template>
  <view class="waiting-page">
    <view class="status-bar"><view class="status-pulse" :class="{ danger: replacementActive }"></view><view class="status-copy"><text>{{ statusTitle }}</text><text>{{ statusSubtitle }}</text></view><text class="status-tag">{{ orderInfo?.status || '待接单' }}</text></view>

    <view class="hero-card">
      <text class="hero-eyebrow">ORDER MATCHING</text>
      <text class="hero-title">{{ replacementActive ? '订单需要重新安排陪玩' : (isTargetedOrder ? '正在等待陪玩师确认' : '正在为你组建队伍') }}</text>
      <text class="hero-sub">{{ replacementActive ? '原订单号和支付记录保持不变，请在下方选择处理方式' : (isTargetedOrder ? '支付已完成，系统已向指定陪玩师发出服务邀请' : '指定邀请与公开抢单同时进行，人数到齐后进入钻石支付') }}</text>
    ...
    <view v-if="orderInfo" class="card progress-card">
      <view class="card-head"><view><text class="card-title">{{ isTargetedOrder ? '服务确认进度' : '接单与入房进度' }}</text><text class="card-sub">{{ isTargetedOrder ? `${readyCount}/1 位指定陪玩已确认服务` : `${readyCount}/${requiredCount} 位陪玩已就位，接单后需在10分钟内进入房间` }}</text></view><text class="open-slot">{{ isTargetedOrder ? (readyCount ? '已确认' : '待确认') : `剩余${Math.max(0, requiredCount - readyCount)}位` }}</text></view>
    ...
    <view class="footer-actions"><button class="ghost-btn" @tap="goMain('home')">返回首页</button><button class="primary-btn" :loading="refreshing" :disabled="refreshing" @tap="handleManualRefresh">{{ refreshing ? '刷新中' : '刷新状态' }}</button><button v-if="orderInfo?.status === '待接单' && !replacementActive" class="danger-btn" @tap="handleCancel">取消订单</button><button v-if="orderInfo?.status === '待支付'" class="primary-btn" @tap="goPayment">去付款</button></view>
  </view>
</template>
...
const statusTitle = computed(() => replacementActive.value ? '指定陪玩已退出' : designations.value.some(item => item.status === 'pending') ? '等待指定陪玩回应' : (isTargetedOrder.value ? '指定服务正在确认' : '订单已派发'))
const statusSubtitle = computed(() => replacementActive.value ? '请重新指定、转公开补位或申请取消剩余服务' : designations.value.some(item => item.status === 'pending') ? (isTargetedOrder.value ? 'TA 接受后即可开始服务；拒绝或超时将自动退款' : '指定邀请接受后，剩余名额继续公开匹配') : (isTargetedOrder.value ? '等待指定陪玩师确认服务' : '陪玩接满后将自动进入付款页面'))
...
function playerRoomText(player: any) { const status = player.room_join_status || 'pending'; if (status === 'confirmed') return '已进入房间'; if (status === 'late_confirmed') return '超时后已进入'; if (status === 'overdue') return '进入房间已超时'; if (status === 'waived') return '管理员已免除'; const deadline = player.room_join_deadline; if (!deadline) return '等待进入房间'; const seconds = Math.max(0, Math.floor((new Date(deadline).getTime() - now.value) / 1000)); return seconds ? `入房 ${String(Math.floor(seconds / 60)).padStart(2, '0')}:${String(seconds % 60).padStart(2, '0')}` : '进入房间已超时' }
function updateWaitTime() { now.value = Date.now(); if (!orderInfo.value?.created_at) { waitTime.value = ''; return }; const diff = Math.max(0, Math.floor((now.value - new Date(orderInfo.value.created_at).getTime()) / 1000)); waitTime.value = `${Math.floor(diff / 60)}:${String(diff % 60).padStart(2, '0')}` }
