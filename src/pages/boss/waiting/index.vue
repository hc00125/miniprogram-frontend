<template>
  <view class="waiting-page">
    <view class="status-bar">
      <view class="status-pulse" :class="{ danger: replacementActive }"></view>
    ...
    <view class="hero-card">
      ...
      <text class="hero-sub">{{ replacementActive ? '原订单号和付款记录保持不变，请在下方选择处理方式' : (isTargetedOrder ? '支付已完成，系统已向指定陪玩师发出服务邀请' : '指定邀请与公开抢单同时进行，人数到齐后进入付款') }}</text>
    ...
    <view class="footer-actions">
      ...
      <button v-if="orderInfo?.status === '待接单' && !replacementActive" class="danger-btn" @tap="handleCancel">取消订单</button>
...
async function checkOrder() {
  ...
  try {
    const [res, designationResult] = await Promise.all([getOrder(orderNo.value), getOrderDesignations(orderNo.value)])
    ...
    else if ((res.status === '待开打' || res.status === '进行中') && !res.replacement?.active) { stopTimers(); replace('/pages/boss/in-progress/index', { orderNo: orderNo.value }) }
    else if (res.status === '进行中' && res.replacement?.active) { stopTimers(); replace('/pages/boss/in-progress/index', { orderNo: orderNo.value }) }
