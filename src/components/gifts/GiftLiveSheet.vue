<template>
  <view class="mask" @tap="close"><view class="sheet" @tap.stop>
    <view class="heading"><text>{{ mode === 'direct' ? `送给 ${recipient.name}` : '购买入背包（不会赠送）' }}</text><button @tap="close">关闭</button></view>
    <view class="tabs"><button @tap="records">我的礼物</button><button :disabled="busy || loading || pending" @tap="changeMode('direct')">购买后赠送</button><button data-action="buy-inventory" :disabled="busy || loading || pending" @tap="changeMode('inventory')">购买入背包</button></view>
    <text class="note">库存专用报价尚未提供：库存仅可查看，禁止赠送；收到礼物不进入可转赠背包。</text>
    <text v-if="loading" class="note">正在加载礼物…</text>
    <text v-if="error" class="warning">{{ error }}</text>
    <text v-if="cap" class="note">{{ cap.blockers.join(' · ') || (cap.purchase_enabled ? '按服务端限制报价，付款时再次校验' : '服务端未开放购买') }}</text>
    ...
    <scroll-view v-else scroll-y class="content">
      <GiftGrid :items="catalog" :selected-code="selected ? selected.code : ''" @select="select" />
    ...
    <view v-if="!pending" class="footer">
      ...
      <button v-if="quote" data-action="pay" class="primary" :disabled="busy || !canPay" @tap="pay">确认支付 {{ quote.total_diamonds }} 钻石{{ mode === 'direct' ? '并赠送' : '购买入背包' }}</button>
      <button v-if="quote && !hasDiamonds(quote.available_diamonds, quote.total_diamonds)" @tap="recharge">前往充值（返回不自动支付）</button>
      <text v-if="result">{{ paymentLabel(result.payment_status) }} · {{ result.purchase_no }}</text>
    </view>
