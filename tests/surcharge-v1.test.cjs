...
test('surcharge host states server limits and missing spendable quote despite server can_submit; no total balance fallback',async()=>{
 ...
 const h=harness({'@/api/commerceRead':{readOrderSurcharge:async()=>d}}),C=h.load('src/components/orders/OrderSurchargeHost.vue').default,scope=vue.effectScope(),props={orderNo:'original'},draw=scope.run(()=>C.setup(props,{expose(){}})),tree=()=>nodes(draw(props,[]))
...
test('real surcharge record action performs one original-detail GET and hides late result',async()=>{
 const reads=[],h=harness({'@/api/commerceRead':{readOrderSurcharge:async()=>({order_no:'original',required_players:2,eligible:false,can_submit:false,disabled_reason:'',blockers:[],paid_diamonds:0,processing_diamonds:10,refunded_diamonds:0,amount_options_diamonds:[],records:[{surcharge_no:'s',payment_status:'unknown',amount_diamonds:10}],count:1,next:null,previous:null})},'@/api/surchargeCommerce':{readSurchargePayment:(...args)=>new Promise(resolve=>reads.push({args,resolve}))}})
