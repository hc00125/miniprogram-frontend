...
async function main(){
 ...
 const requests=[],f='components/orders/OrderSurchargeHost',h=runtime(f,{commerceRead:{readOrderSurcharge:(...args)=>new Promise(resolve=>requests.push({args,resolve}))}}),props=vue.reactive({orderNo:'original/订单'}),render=h.setup(props),refresh=event(f,'refresh','catchtap')
