// Manual upload to WeChat mini-program CI
// Bypasses miniprogram-ci's broken compile pipeline
const fs = require('fs');
...
// Step 2: Pack into CI binary format
...
async function main() {
  try {
    console.log('=== WeChat Mini-Program Manual Upload ===\n');
    ...
    // Add signature file to the package
    console.log('[4/6] Adding ci.signature to package...');
    ...
    const uploadUrl = DOMAIN + '/wxa/ci/upload?' + queryParams.toString();

    // Upload
    console.log('[6/6] Uploading to WeChat...');
    console.log(`       URL: ${uploadUrl.substring(0, 80)}...`);
    console.log(`       This may take 20-60 seconds...`);

    const uploadResp = await httpPost(uploadUrl, gzipped, 'application/octet-stream');
    
    if (uploadResp.status === 200) {
      console.log(`\n✅ Upload response (${uploadResp.status}):`);
      try {
        ...
        if (parsed.errCode === 0) {
          ...
          console.log('\n⚠️ Upload returned error:', parsed.errMsg || parsed.errCode);
        }
      } catch {
        console.log(uploadResp.body.substring(0, 500));
      }
    } else {
      console.log(`\n❌ HTTP ${uploadResp.status}: ${uploadResp.body.substring(0, 200)}`);
    }
